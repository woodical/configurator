...
def default_config():
    """
    Held in a common/shared package.
...
def get_config(config_name):
    ...
    if config_name not in configs:
        config = parse_properties_file(config_name)
        if "mkv" in config:
            config = config.pop("mkv")

        config_name = config["generic"]["profile_name"]

        overrides = config

    return merge_dicts(
        default_config()[config_name], app_config(), configs[config_name], overrides
    )
